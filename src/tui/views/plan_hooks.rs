// Plan-hook attachment sub-view (TUI-plan.md §1, step 34).
//
// A focused sub-view over the plan-detail screen that lets the user inspect,
// add, and remove plan-wide hook attachments. Entered via the `/plan
// set-hook|hooks` palette commands (or the `H` keybinding).
//
// State machine: three modes. `List` shows the current plan-wide attachments
// grouped by lifecycle as a `ratatui::Table`. `a` opens `LifecyclePicker`
// which steps through the four `hook_library::Lifecycle` variants (pre-step,
// post-step, pre-test, post-test); selecting one transitions to
...
//
// The view is split into a pure state machine (`PlanHooksApp`) and a renderer
// (`render`) so we can drive the state machine in tests without a real
// terminal. All write-throughs (attach / detach via `storage::*`) live in the
// dispatcher loop alongside `run_plan_dependencies_tui`; this module only
...
/// One plan-wide hook attachment row, projected from
/// [`crate::storage::StepHookRow`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlanHookRef {
...
/// [`PlanHooksApp::new`] / [`PlanHooksApp::refresh`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// What [`PlanHooksApp::handle_key`] returns each turn. The dispatcher loop
...
pub enum Outcome {
    ...
    /// attachment via [`crate::storage::attach_hook_to_plan`].
    AddRequested {
        lifecycle: Lifecycle,
        hook_name: String,
    },
    /// User pressed `d` on a row — request detachment via
    /// [`crate::storage::detach_hook`] with `step_id == None`.
    RemoveRequested {
        lifecycle: Lifecycle,
        hook_name: String,
    },
...
/// Sub-view state.
pub struct PlanHooksApp {
    /// Plan whose hooks are being edited.
    pub plan_id: String,
    /// Display slug of the plan, used in the title bar.
    pub plan_slug: String,
    /// Current plan-wide attachments, sorted by `(lifecycle, hook_name)` so
    /// the table is stable across refreshes.
    pub attachments: Vec<PlanHookRef>,
...
impl PlanHooksApp {
    ...
    pub fn refresh(&mut self, attachments: Vec<PlanHookRef>, all_hooks: Vec<HookCandidate>) {
        let mut attachments = attachments;
        sort_attachments(&mut attachments);
...
fn sort_attachments(rows: &mut [PlanHookRef]) {
    rows.sort_by(|a, b| {
...
pub fn render(frame: &mut Frame, area: Rect, app: &mut PlanHooksApp) {
    if area.width == 0 || area.height == 0 {
        return;
    }

    // -- Title + hint row --------------------------------------------------
    let title = format!(" Plan hooks — {} ", app.plan_slug);
    ...
    if app.attachments.is_empty() {
        let para = Paragraph::new(Line::from(Span::styled(
            "(no plan-wide hooks — press `a` to attach one)",
...
fn render_lifecycle_picker(frame: &mut Frame, area: Rect, app: &PlanHooksApp) {
...
mod tests {
    ...
    fn att(lifecycle: Lifecycle, name: &str) -> PlanHookRef {
        PlanHookRef {
            lifecycle,
    ...
    fn render_list_shows_lifecycle_label_for_each_variant() {
        ...
        assert!(s.contains("demo"), "title should include slug");
    }
    ...
    fn end_to_end_remove_persists_through_storage_each_lifecycle() {
        for lifecycle in LIFECYCLES {
            ...
            let removed =
                storage::detach_hook(&conn, &plan_id, None, got_lifecycle.as_str(), &got_name)
                    .unwrap();
